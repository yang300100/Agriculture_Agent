...
class FieldBoundary(BaseModel):
    ...
    soil_type: str = ""
    current_crop: str = ""
    history: List[Dict[str, Any]] = Field(default_factory=list)  # 种植历史
...
class MapManager:
    ...
    def add_planting_history(self, field_id: str, crop: str, season: str = "",
                             yield_amount: float = 0, notes: str = ""):
        field = self.get_field(field_id)
        if not field:
            return False
        entry = {
            "crop": crop,
            "season": season or datetime.now().strftime("%Y"),
            "yield_amount": yield_amount,
            "notes": notes,
            "recorded_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        }
        field.history = field.history or []
        field.history.append(entry)
        field.updated_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self._save_data()
        return True

    def get_field_history(self, field_id: str) -> List[Dict]:
        field = self.get_field(field_id)
        return sorted(field.history or [], key=lambda h: h.get("recorded_at", ""), reverse=True)
        ...
        return [
            {
                ...
                "history_crops": [h["crop"] for h in (f.history or [])],
                "history_count": len(f.history or []),
            }
            for f in self.fields
        ]
...
def get_location_from_address(address: str) -> Optional[Tuple[float, float]]:
    """将地址转换为坐标，内置国内常用地名映射，Nominatim 仅兜底"""
    builtin = {
        ...
        "河北": (38.04, 114.50), "山西": (37.87, 112.55), "内蒙古": (40.84, 111.75),
        "辽宁": (41.80, 123.43), "吉林": (43.90, 125.32), "黑龙江": (45.80, 126.53),
        "江苏": (32.06, 118.78), "浙江": (30.28, 120.15), "安徽": (31.86, 117.28),
        "福建": (26.07, 119.30), "江西": (28.68, 115.86), "山东": (36.67, 116.98),
        "河南": (34.76, 113.66), "湖北": (30.60, 114.30), "湖南": (28.23, 112.97),
        "广东": (23.13, 113.26), "广西": (22.82, 108.37), "海南": (20.03, 110.32),
        "四川": (30.67, 104.07), "贵州": (26.57, 106.71), "云南": (24.88, 102.83),
        "西藏": (29.65, 91.13), "陕西": (34.26, 108.94), "甘肃": (36.06, 103.83),
        "青海": (36.62, 101.78), "宁夏": (38.47, 106.27), "新疆": (43.82, 87.62),
        "华北": (39.90, 116.40), "东北": (43.90, 125.32), "华东": (32.06, 118.78),
        "华中": (30.60, 114.30), "华南": (23.13, 113.26), "西南": (30.67, 104.07),
        "西北": (36.06, 103.83),
    }
