            ...
            if filename.endswith('.json'):
                ...
                try:
                    with open(filepath, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                        # 统一字段名：JSON 用 crop_name，dataclass 用 name
                        if 'crop_name' in data and 'name' not in data:
                            data['name'] = data.pop('crop_name')
                        crop_info = CropInfo(**{k: v for k, v in data.items()
                                               if k in CropInfo.__dataclass_fields__})
                        self.crops[data['name']] = crop_info
                        # 同时添加别名映射
    ...
    def _generate_stage_tasks(self, crop_info: CropInfo,
        ...
        if sowing_date:
            try:
                ...
                logger.warning("无法解析播种日期: %s", sowing_date)
